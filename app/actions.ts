...
export async function createPlayerCard(formData: FormData){requireAdmin(); const data=playerCardSchema.parse(Object.fromEntries(formData)); await prisma.playerCard.create({data:{...data,slug:slugifyNickname(data.nickname),cardType:getCardTypeByRating(data.rating) as any,photoUrl:data.photoUrl||null,rank:data.rank||null}}); revalidatePath('/cards'); revalidatePath('/admin/cards');}
export async function updatePlayerCard(id:string, formData:FormData){requireAdmin(); const existing=await prisma.playerCard.findUnique({where:{id}}); const data=playerCardSchema.parse(Object.fromEntries(formData)); await prisma.playerCard.update({where:{id},data:{...data,slug:slugifyNickname(data.nickname),cardType:getCardTypeByRating(data.rating) as any,photoUrl:data.photoUrl||null,rank:data.rank||null}}); revalidatePath('/cards'); if(existing) revalidatePath(`/cards/${existing.slug}`); revalidatePath(`/cards/${slugifyNickname(data.nickname)}`);}
